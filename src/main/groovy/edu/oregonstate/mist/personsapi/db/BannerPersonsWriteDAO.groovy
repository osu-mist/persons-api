...
public interface BannerPersonsWriteDAO extends Closeable {
    ...
    @OutParameter(name = "return_value", sqlType = Types.VARCHAR)
    OutParameters updateJob(@Bind('osuID') String osuID,
                            @BindJob JobObject job)

    @SqlCall(AbstractPersonsDAO.laborChangeUpdateFunction)
    @OutParameter(name = "return_value", sqlType = Types.VARCHAR)
    OutParameters updateLaborChangeJob(@Bind('osuID') String osuID,
                                       @BindJob JobObject job)
